...
public final class ImageBuffer {
    ...
    public func handleMemoryPressure() {
        ...
        StillroomLog.loading.notice("Memory pressure: budget now \(self.configuration.maxDecodedBytes) bytes")
    }
    ...
    private func preemptLowestPriorityPrefetch() -> Bool {
        for id in priorities.reversed() where !protected.contains(id) {
            ...
            entry.status = .queued
            entries[id] = entry
            StillroomLog.loading.debug("Preempted prefetch \(id.logToken)")
            return true
        }
        return false
    }
    ...
    private func start(_ id: AssetID) {
        ...
        entries[id] = entry
        StillroomLog.loading.info("Request \(id.logToken) attempt \(attempt)")
    ...
    private func finish(id: AssetID, token: UInt64, result: Result<LoadedImage, any Error>) {
        guard var entry = entries[id], entry.token == token, entry.isLoading else {
            ...
            StillroomLog.loading.info("Ignored stale result for \(id.logToken)")
            return
        }
        entry.watchdog?.cancel()
        entry.watchdog = nil
    ...
    private func enforceByteBudget() {
        ...
        for id in priorities.reversed() where !protected.contains(id) {
            ...
            StillroomLog.loading.debug("Evicted \(id.logToken) for byte budget")
        }
    }
