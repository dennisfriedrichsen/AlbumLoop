...
public struct StillroomLogger: Sendable {
    private let logger: Logger
    private let category: String

    init(category: String) {
        self.category = category
        self.logger = Logger(subsystem: StillroomLog.subsystem, category: category)
    }
...
/// Bounded on-device log file: `stillroom.log`, rotated to `stillroom.1.log`
...
public final class DiagnosticFileLog: @unchecked Sendable {
    ...
    public static let fileName = "stillroom.log"

    // All mutable state is confined to `queue`.
    private let queue = DispatchQueue(label: "Stillroom.DiagnosticFileLog")
    ...
    private func rotate() {
        ...
        try? handle?.close()
        handle = nil
        let old = directory.appendingPathComponent("stillroom.1.log")
